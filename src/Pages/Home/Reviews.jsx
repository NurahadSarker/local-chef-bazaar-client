// import React from 'react';
// import Avatar from '../../assets/avatar.png'
// import { FaStar, FaStarHalfAlt } from 'react-icons/fa';
// import { RiDoubleQuotesL } from 'react-icons/ri';
// import Marquee from 'react-fast-marquee';

// const Reviews = () => {
//     return (
//         <div className='bg-base-300 pt-15'>
//             <div className='max-w-[1200px] mx-auto mb-15'>
//                 <div>
//                     <h1 className='text-[32px] font-bold text-center mb-2'>What Our Food Lovers Are Saying</h1>
//                     <p className='text-center'>Real experiences from people who enjoy fresh, homemade meals from trusted local chefs.</p>
//                 </div>
//                 <Marquee className='gap-5 h-80'>
//                     <div className='flex gap-5'>
//                         <div className='bg-base-100 w-80 p-5 shadow-md rounded-md'>
//                             <div>
//                                 <div className='bg-base-300 w-15 rounded-full border-2 border-base-100 -mt-12'>
//                                     <img className='w-15 rounded-full' src={Avatar} alt="" />
//                                 </div>
//                                 <div>
//                                     <h1 className='text-[18px] font-medium'>Md. Nur-Ahad Robi</h1>
//                                     <div className='flex items-center justify-between'>
//                                         <div className='flex items-center gap-2 text-yellow-400'>
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStarHalfAlt />
//                                         </div>
//                                         <div>
//                                             <h1>10-10-2025</h1>
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
//                             <div className='mb-2'>
//                                 <div className='text-2xl'>
//                                     <RiDoubleQuotesL />
//                                 </div>
//                                 <div className='border border-gray-400 ml-6 -mt-3'></div>
//                             </div>
//                             <div>
//                                 <p>Lorem ipsum dolor, sit amet adipisicing elit. Dolor, ut.</p>
//                             </div>
//                         </div>
//                         <div className='bg-base-100 w-80 p-5 shadow-md rounded-md'>
//                             <div>
//                                 <div className='bg-base-300 w-15 rounded-full border-2 border-base-100 -mt-12'>
//                                     <img className='w-15 rounded-full' src={Avatar} alt="" />
//                                 </div>
//                                 <div>
//                                     <h1 className='text-[18px] font-medium'>Md. Nur-Ahad Robi</h1>
//                                     <div className='flex items-center justify-between'>
//                                         <div className='flex items-center gap-2 text-yellow-400'>
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStarHalfAlt />
//                                         </div>
//                                         <div>
//                                             <h1>10-10-2025</h1>
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
//                             <div className='mb-2'>
//                                 <div className='text-2xl'>
//                                     <RiDoubleQuotesL />
//                                 </div>
//                                 <div className='border border-gray-400 ml-6 -mt-3'></div>
//                             </div>
//                             <div>
//                                 <p>Lorem ipsum dolor, sit amet adipisicing elit. Dolor, ut.</p>
//                             </div>
//                         </div>
//                         <div className='bg-base-100 w-80 p-5 shadow-md rounded-md'>
//                             <div>
//                                 <div className='bg-base-300 w-15 rounded-full border-2 border-base-100 -mt-12'>
//                                     <img className='w-15 rounded-full' src={Avatar} alt="" />
//                                 </div>
//                                 <div>
//                                     <h1 className='text-[18px] font-medium'>Md. Nur-Ahad Robi</h1>
//                                     <div className='flex items-center justify-between'>
//                                         <div className='flex items-center gap-2 text-yellow-400'>
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStarHalfAlt />
//                                         </div>
//                                         <div>
//                                             <h1>10-10-2025</h1>
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
//                             <div className='mb-2'>
//                                 <div className='text-2xl'>
//                                     <RiDoubleQuotesL />
//                                 </div>
//                                 <div className='border border-gray-400 ml-6 -mt-3'></div>
//                             </div>
//                             <div>
//                                 <p>Lorem ipsum dolor, sit amet adipisicing elit. Dolor, ut.</p>
//                             </div>
//                         </div>
//                         <div className='bg-base-100 w-80 p-5 shadow-md rounded-md'>
//                             <div>
//                                 <div className='bg-base-300 w-15 rounded-full border-2 border-base-100 -mt-12'>
//                                     <img className='w-15 rounded-full' src={Avatar} alt="" />
//                                 </div>
//                                 <div>
//                                     <h1 className='text-[18px] font-medium'>Md. Nur-Ahad Robi</h1>
//                                     <div className='flex items-center justify-between'>
//                                         <div className='flex items-center gap-2 text-yellow-400'>
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStarHalfAlt />
//                                         </div>
//                                         <div>
//                                             <h1>10-10-2025</h1>
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
//                             <div className='mb-2'>
//                                 <div className='text-2xl'>
//                                     <RiDoubleQuotesL />
//                                 </div>
//                                 <div className='border border-gray-400 ml-6 -mt-3'></div>
//                             </div>
//                             <div>
//                                 <p>Lorem ipsum dolor, sit amet adipisicing elit. Dolor, ut.</p>
//                             </div>
//                         </div>
//                         <div className='bg-base-100 w-80 p-5 shadow-md rounded-md'>
//                             <div>
//                                 <div className='bg-base-300 w-15 rounded-full border-2 border-base-100 -mt-12'>
//                                     <img className='w-15 rounded-full' src={Avatar} alt="" />
//                                 </div>
//                                 <div>
//                                     <h1 className='text-[18px] font-medium'>Md. Nur-Ahad Robi</h1>
//                                     <div className='flex items-center justify-between'>
//                                         <div className='flex items-center gap-2 text-yellow-400'>
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStar />
//                                             <FaStarHalfAlt />
//                                         </div>
//                                         <div>
//                                             <h1>10-10-2025</h1>
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
//                             <div className='mb-2'>
//                                 <div className='text-2xl'>
//                                     <RiDoubleQuotesL />
//                                 </div>
//                                 <div className='border border-gray-400 ml-6 -mt-3'></div>
//                             </div>
//                             <div>
//                                 <p>Lorem ipsum dolor, sit amet adipisicing elit. Dolor, ut.</p>
//                             </div>
//                         </div>
//                     </div>
//                 </Marquee>
//             </div>
//         </div>
//     );
// };

// export default Reviews;

import React, { useEffect, useState } from 'react';
import Avatar from '../../assets/avatar.png';
import { FaStar, FaStarHalfAlt } from 'react-icons/fa';
import { RiDoubleQuotesL } from 'react-icons/ri';
import Marquee from 'react-fast-marquee';
import useAxiosSecure from '../../Hooks/useAxiosSecure';

const Reviews = () => {
    const axiosSecure = useAxiosSecure();
    const [reviews, setReviews] = useState([]);

    useEffect(() => {
        axiosSecure
            .get('/reviews/home')
            .then(res => setReviews(res.data))
            .catch(err => console.error(err));
    }, [axiosSecure]);

    return (
        <div className='bg-base-300 pt-15'>
            <div className='max-w-[1200px] mx-auto mb-15'>
                <div>
                    <h1 className='text-[32px] font-bold text-center mb-2'>
                        What Our Food Lovers Are Saying
                    </h1>
                    <p className='text-center'>
                        Real experiences from people who enjoy fresh, homemade meals from trusted local chefs.
                    </p>
                </div>

                <Marquee className='gap-5 h-80 pauseOnHover={true}'>
                    <div className='flex gap-5'>
                        {reviews.map(review => (
                            <div
                                key={review._id}
                                className='bg-base-100 w-80 p-5 shadow-md rounded-md'
                            >
                                <div>
                                    <div className='bg-base-300 w-15 rounded-full border-2 border-base-100 -mt-12'>
                                        <img
                                            className='w-15 h-15 rounded-full'
                                            src={review.userPhoto || Avatar}
                                            alt=""
                                        />
                                    </div>

                                    <div>
                                        <h1 className='text-[18px] font-medium'>
                                            {review.userName || "Anonymous"}
                                        </h1>

                                        <div className='flex items-center justify-between'>
                                            <div className='flex items-center gap-1 text-yellow-400'>
                                                {[...Array(5)].map((_, i) =>
                                                    i + 1 <= Math.floor(review.rating) ? (
                                                        <FaStar key={i} />
                                                    ) : (
                                                        <FaStarHalfAlt key={i} />
                                                    )
                                                )}
                                            </div>

                                            <div>
                                                <h1 className='text-sm'>
                                                    {new Date(review.createdAt).toLocaleDateString()}
                                                </h1>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div className='mb-2'>
                                    <div className='text-2xl'>
                                        <RiDoubleQuotesL />
                                    </div>
                                    <div className='border border-gray-400 ml-6 -mt-3'></div>
                                </div>

                                <div>
                                    <p className='text-sm'>
                                        {review.review}
                                    </p>
                                </div>
                            </div>
                        ))}
                    </div>
                </Marquee>
            </div>
        </div>
    );
};

export default Reviews;
